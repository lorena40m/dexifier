...
export default function SwapPage() {
  const { state, swapData, tokenFrom, tokenTo, amountFrom, selectedRoute, isMobile } = useDexifier();

  return (
    <main className={cn("relative min-h-screen pt-32 md:bg-[url('/assets/background.jpg')] bg-cover", isMobile ? "px-8" : "px-4")}>
      ...
      <section className={cn("flex justify-center", isMobile ? "h-full flex-col gap-8" : "flex-row flex-wrap gap-5 h-[560px] items-stretch m-4 mt-32")}>
        <DexifierCard />
          ...
          <AnimatePresence>
            {tokenFrom && tokenTo && amountFrom ?
              <motion.div
                ...
                animate={isMobile ? { height: "100%", opacity: 1 } : { width: 650, opacity: 1 }}
      ...
      {isMobile && <div className="flex-1 py-12">
        <ConfirmModal>
          <Button
