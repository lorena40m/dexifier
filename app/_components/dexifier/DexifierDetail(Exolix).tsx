...
import { ExTxInfo, RateResponse } from "@/app/types/exolix";
...
interface StepStateProps {
  state: ExTxInfo['status'], // Pending swap state
}

type ExolixData = {
  swapStatus: ExTxInfo;
  selectedRoute: RateResponse;
};

const DexifierDetailExolix = () => {
  const { swapStatus, selectedRoute } = useDexifier() as ExolixData;
  const { tokenFrom, tokenTo } = useDexifier();
  const { initialize } = useDexifier();
  const isFinished = useMemo(() => swapStatus && (swapStatus.status in ["success", "overdue", "refunded"]), [swapStatus])
  ...
  return (
    <Card className="max-w-[650px] min-h-[540px] w-full h-full bg-modal/5 border border-[#AAA]/20 backdrop-blur-lg p-2 rounded-[2rem] shadow-lg text-white">
      ...
      <CardContent className="overflow-auto h-[380px] px-6">
        <div className="mb-8">
          {swapStatus && <div className="flex justify-between">
            <span className="text-lg font-semibold">
              {`${swapStatus.status === "success" ? "Finished at" : "Created at"}:`}
            </span>
        ...
        {swapStatus && selectedRoute && tokenFrom && tokenTo && <div className="mb-7 flex flex-col items-center text-xs">
          <div className="flex gap-1">
            <SwapToken
              token={tokenFrom}
              amount={selectedRoute.fromAmount.toString()}
              variant={`default`}
            ...
            <div className="relative border-t border-dashed min-w-24 flex-grow mt-8">
              ...
              >
                <TooltipTemplate content={`Exolix`}>
                  <TokenIcon
                    token={{ image: `https://docs.Exolix.io/Exolixlogo.png`, className: "size-6" }}
                  />
                </TooltipTemplate>
              </div>
            </div>
            <SwapToken
              token={tokenTo}
              amount={selectedRoute.toAmount.toString()}
              variant={`default`}
              className="flex flex-col"
            />
          </div>
          ...
          <div className="mt-4 flex items-center gap-1">
            ...
            <div className="relative border-t border-dashed w-full min-w-24 h-[1px] flex-grow" >
              <div className="absolute w-full -top-3 flex justify-center">
                <TooltipTemplate content={`Exolix`}>
                  <TokenIcon
                    token={{ image: `https://docs.Exolix.io/Exolixlogo.png`, className: "size-4" }}
                  />
                </TooltipTemplate>
              </div>
            </div>
            ...
            <div className="text-white/50 ml-2 col-span-2">
              <StepState state={swapStatus.status} />
            </div>
          </div>
        </div>}
      </CardContent>
...
const SwapToken: FC<SwapTokenProps> = ({ token, amount, className, variant }) => {
  ...
  const blockchain: BlockchainMeta = blockchains.find((blockchain: BlockchainMeta) => formatChainName(blockchain.name) === token.blockchain)
