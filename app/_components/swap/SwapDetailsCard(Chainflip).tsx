...
const SwapDetailsCardChainflip = () => {
  const { depositData, selectedQuote, initializeQuote } = useQuote();
  const isFinished = useMemo(() => depositData && (depositData.state === "COMPLETED" || depositData.state === "FAILED"), [depositData])
  ...
  return (
    <Card className="max-w-[650px] min-h-[540px] w-full h-full bg-modal/5 border border-[#AAA]/20 backdrop-blur-lg p-2 rounded-[2rem] shadow-lg text-white">
      ...
      <CardContent className="overflow-auto h-[380px] px-6">
        <div className="mb-8">
          {depositData && <div className="flex justify-between">
            <span className="text-lg font-semibold">
              {`${depositData.state === "COMPLETED" ? "Finished at" : "Created at"}:`}
            </span>
        ...
        {depositData && selectedQuote && <div className="mb-7 flex flex-col items-center text-xs">
          <div className="flex gap-1">
            <SwapToken
              asset={selectedQuote.srcAsset.asset}
              chain={selectedQuote.srcAsset.chain}
              amount={selectedQuote.depositAmount}
              variant={`default`}
            ...
            <SwapToken
              asset={selectedQuote.destAsset.asset}
              chain={selectedQuote.destAsset.chain}
              amount={selectedQuote.egressAmount}
              variant={`default`}
              className="flex flex-col"
            />
          </div>
          ...
          <div className="mt-4 flex items-center gap-1">
            ...
            <div className="text-white/50 ml-2 col-span-2">
              <StepState state={depositData.state} />
            </div>
          </div>
        </div>}
      </CardContent>
...
export default SwapDetailsCardChainflip;